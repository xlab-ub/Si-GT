...
def compute_posenc_stats(data, pe_types, is_undirected, cfg):
    ...
    # === LapPE or EquivStableLapPE ===
    if 'LapPE' in pe_types or 'EquivStableLapPE' in pe_types:
        key = 'posenc_LapPE' if 'LapPE' in pe_types else 'posenc_EquivStableLapPE'
        lap_cfg = cfg[key]['eigen']
        laplacian_norm_type = lap_cfg['laplacian_norm'].lower()
        ...
        max_freqs = lap_cfg['max_freqs']
        eigvec_norm = lap_cfg['eigvec_norm']
        data.EigVals, data.EigVecs = get_lap_decomp_stats(evals, evects, max_freqs, eigvec_norm)

    # === SignNet ===
    if 'SignNet' in pe_types:
        sn_cfg = cfg['posenc_SignNet']['eigen']
        norm_type = sn_cfg['laplacian_norm'].lower()
        if norm_type == 'none':
            norm_type = None
        L = to_scipy_sparse_matrix(
        ...
        data.eigvals_sn, data.eigvecs_sn = get_lap_decomp_stats(
            evals=evals_sn,
            evects=evects_sn,
            max_freqs=sn_cfg['max_freqs'],
            eigvec_norm=sn_cfg['eigvec_norm']
        )

    # === RWSE ===
    if 'RWSE' in pe_types:
        rwse_cfg = cfg['posenc_RWSE']
        ksteps = rwse_cfg['kernel']['times']
        rw_landing = get_rw_landing_probs(ksteps=ksteps, edge_index=data.edge_index, num_nodes=N)
        data.pestat_RWSE = rw_landing

    # === HKdiagSE or HKfullPE ===
    if 'HKdiagSE' in pe_types or 'HKfullPE' in pe_types:
        ...
        if 'HKfullPE' in pe_types:
            raise NotImplementedError("HKfullPE is not implemented due to padding limitations.")
        if 'HKdiagSE' in pe_types:
            hk_cfg = cfg['posenc_HKdiagSE']
            kernel_times = hk_cfg['kernel']['times']
            hk_diag = get_heat_kernels_diag(evects_heat, evals_heat, kernel_times, space_dim=0)
            data.pestat_HKdiagSE = hk_diag

    # === Electrostatic ===
    if 'ElstaticSE' in pe_types:
        data.pestat_ElstaticSE = get_electrostatic_function_encoding(undir_edge_index, N)
